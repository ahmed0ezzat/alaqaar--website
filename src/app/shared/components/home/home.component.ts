...
export class HomeComponent implements OnInit {
  ...
  activeCity: number = 1
  cites: any[] = [
    { id: 1, name: "New Cairo", disabled: false }
  ]
  areas = [
    { id: 1, name: "area", disabled: false }
  ]
  neighborhood = [
    { id: 1, name: "New Cairo", disabled: false },
  ]
  ...
  subString(str: any, strlength:number = 30) {
    var div = document.createElement("div");
    div.innerHTML = str;
    let text = div.textContent || div.innerText || "";
    return text.length && text.length > strlength ? text.substring(0, strlength) + '...' : text
  }
  ...
  getName(itemID: any, itemsArray: any[]) {
    let item: any =  itemsArray.filter(item => item.id === itemID)
    return item[0].name
  }
  ...
  checkIfSelected(itemID: any, itemsArray: any) {
    return itemsArray && itemsArray.length > 0 && itemsArray.includes(itemID) ? true : false
  }

  onNgOptionChanged(items: any,cites: any[]) {
    cites.map(city => city.selected = false)
    if (items && items.length > 0) {
      cites.map(city => {
        if (items.includes(city.id)) {
           city.selected = true
        }
      })
    }
  ...
  showMore() {
    return this.showMoreControls = true;
  }
  getSelectedAreas(ids: any) {
    return this.cites.filter(city => ids.includes(city.id))
  }
  getselectedNeighborhood(ids: any) {
    return this.neighborhood.filter((neighborhood: any) => ids.includes(neighborhood.id))
  }
  getselectedrealStateTypes(ids: any) {
    return this.RealEstateType.filter((type: any) => ids.includes(type.id))
  }
  deleteItem(id: number, type: string) {
    switch (type) {
      case 'area':
        return this.selectedArea = this.selectedArea.filter((areaid: any) => areaid !== id)
      case 'neighborhood':
        return this.selectedNeighborhood = this.selectedNeighborhood.filter((neighborhood: any) => neighborhood !== id)
      case 'realEstateType':
        return this.SelectedRealEstateType = this.defaultSelectedRealEstateType
    }
  }
}
